...
GMenu2X::GMenu2X()
{
	...
	if (fwType == "open2x")
	{
		...
		case VOLUME_MODE_MUTE:
			setVolumeScaler(VOLUME_SCALER_MUTE);
			break;
		case VOLUME_MODE_PHONES:
			setVolumeScaler(volumeScalerPhones);
			break;
		case VOLUME_MODE_NORMAL:
			setVolumeScaler(volumeScalerNormal);
			break;
		}
	}
#if defined(TARGET_GP2x)
...
void GMenu2X::readConfig()
{
	...
	if (!confStr["wallpaper"].empty() && !fileExists(confStr["wallpaper"]))
		confStr["wallpaper"] = "";
	if (confStr["skin"].empty() || !fileExists("skins/" + confStr["skin"]))
...
void GMenu2X::readConfigOpen2x()
{
	...
	while (getline(inf, line, '\n'))
	{
		...
		if (name == "USB_NET_ON_BOOT")
			o2x_usb_net_on_boot = value == "y" ? true : false;
		else if (name == "USB_NET_IP")
			o2x_usb_net_ip = value;
		else if (name == "TELNET_ON_BOOT")
			o2x_telnet_on_boot = value == "y" ? true : false;
		else if (name == "FTP_ON_BOOT")
			o2x_ftp_on_boot = value == "y" ? true : false;
		else if (name == "GP2XJOY_ON_BOOT")
			o2x_gp2xjoy_on_boot = value == "y" ? true : false;
		else if (name == "USB_HOST_ON_BOOT")
			o2x_usb_host_on_boot = value == "y" ? true : false;
		else if (name == "USB_HID_ON_BOOT")
			o2x_usb_hid_on_boot = value == "y" ? true : false;
		else if (name == "USB_STORAGE_ON_BOOT")
			o2x_usb_storage_on_boot = value == "y" ? true : false;
		else if (name == "VOLUME_MODE")
			volumeMode = savedVolumeMode = constrain(atoi(value.c_str()), 0, 2);
		else if (name == "PHONES_VALUE")
			volumeScalerPhones = constrain(atoi(value.c_str()), 0, 100);
		else if (name == "NORMAL_VALUE")
			volumeScalerNormal = constrain(atoi(value.c_str()), 0, 150);
	}
	inf.close();
}
void GMenu2X::writeConfigOpen2x()
{
	...
	if (inf.is_open())
	{
		...
		if (volumeScalerPhones != VOLUME_SCALER_PHONES)
			inf << "PHONES_VALUE=" << volumeScalerPhones << endl;
		if (volumeScalerNormal != VOLUME_SCALER_NORMAL)
...
void GMenu2X::readTmp()
{
	...
	while (getline(inf, line, '\n'))
	{
		...
		else if (name == "skipAskUDC") //TonyJih
			skipAskUDC = atoi(value.c_str());
	}
	inf.close();
...
void GMenu2X::main()
{
	...
	while (!quit)
	{
		...
		{
		case VOLUME_MODE_MUTE:
			sc.skinRes("imgs/mute.png")->blit(s, resX - ((19 * 2) + battMsgWidth), bottomBarIconY);
			break;
		default:
			sc.skinRes("imgs/volume.png")->blit(s, resX - ((19 * 2) + battMsgWidth), bottomBarIconY);
			break;
		}
		...
		if (fwType == "open2x")
		{
			...
			case VOLUME_MODE_MUTE:
				...
				break;
			case VOLUME_MODE_PHONES:
				sc.skinRes("imgs/phones.png")->blit(s, resX - 56, bottomBarIconY);
				break;
			default:
				sc.skinRes("imgs/volume.png")->blit(s, resX - 56, bottomBarIconY);
				break;
			}
		}
#endif
	...
	while (!close)
	{
		...
		if (f200)
		{
			...
			if (ts.released())
			{
				if (!ts.inRect(box))
					close = true;
				else if (ts.getX() >= selbox.x && ts.getX() <= selbox.x + selbox.w)
					for (i = 0; i < voices.size(); i++)
					{
						selbox.y = box.y + 4 + h * i;
						if (ts.getY() >= selbox.y && ts.getY() <= selbox.y + selbox.h)
						{
							voices[i].action();
							close = true;
							i = voices.size();
